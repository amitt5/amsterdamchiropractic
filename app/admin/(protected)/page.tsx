...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold mb-6" style={{ color: '#191919' }}>Dashboard</h1>
      ...
      {/* Recent */}
      <h2 className="text-base font-semibold mb-4" style={{ color: '#191919' }}>
        Recent appointments
      </h2>
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div
      ...
      style={{ background: '#fff', border: '1px solid #e5e5e5' }}
    >
      <p className="text-3xl font-bold" style={{ color: '#45321A' }}>{value}</p>
...
function Td({ children }: { children: React.ReactNode }) {
  return (
    <td className="px-4 py-3" style={{ color: '#403F3F' }}>
      {children}
    </td>
  )
}
